...
void handlePayloads(AsyncWebServerRequest *request) {
  ...
  output +=  "<a onclick=\"setpayload('disableupdates.bin','Disable Updates','" + String(USB_WAIT) + "')\"><button class=\"btn\">Disable Updates</button></a>&nbsp;";
#endif
...
void setup(){
  ...
  server.onNotFound([](AsyncWebServerRequest *request){
    ...
    if (path.endsWith("goldhen.bin"))
    {
        ...
        response->addHeader("Content-Encoding", "gzip");
        request->send(response);
        AsyncWebServerResponse *response1 = request->beginResponse_P(200, "application/octet-stream", app2usb_gz, sizeof(app2usb_gz));
        response1->addHeader("Content-Encoding", "gzip");
        request->send(response1);
        AsyncWebServerResponse *response2 = request->beginResponse_P(200, "application/octet-stream", appdumper_gz, sizeof(appdumper_gz));
        response2->addHeader("Content-Encoding", "gzip");
        request->send(response2);
        AsyncWebServerResponse *response3 = request->beginResponse_P(200, "application/octet-stream", disableupdates_gz, sizeof(disableupdates_gz));
        response3->addHeader("Content-Encoding", "gzip");
        request->send(response3);
         AsyncWebServerResponse *response4 = request->beginResponse_P(200, "application/octet-stream", historyblocker_gz, sizeof(historyblocker_gz));
        response4->addHeader("Content-Encoding", "gzip");
        request->send(response4);
        return;
    }
#endif
